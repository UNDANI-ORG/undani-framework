...
(function($) {
    ...
    function AddComment(i) {
        if ($('[data-chat-input]').val() !== '') {
            $.ajax({
                ...
                type: 'GET',
                data: {
                    activityInstanceRefId: i,
                    comment: $('[data-chat-input]').val()
                },
                url: urlApiTracking + '/Execution/Flow/AddComment'
            })
                .done(function (result, textStatus, jqXHR) {
                    var el = $('[data-chat-input]').attr('data-chat-conversation');
                    $(el).append('<div class="message clearfix">' +
                        '<div class="chat-bubble from-me">' + $('[data-chat-input]').val() +
                        '</div></div>');
                    $('[data-chat-input]').val('');
                })
                .fail(function (jqXHR, textStatus, errorThrown) {
                    ///TODO: Modificar undani.message
                })
  ...
  $('[data-chat-input]').on('keypress',function(e){
    if(e.which == 13) {
        AddComment($('#activity').attr('data-ref-id'));
    }
  });

  $('[data-chat-send]').on('click', function (e) {
        AddComment($('#activity').attr('data-ref-id'));
  });
